...
const STRATEGY_DESCRIPTIONS: Record<string, string> = {
    leek: "每支买入 $50,000 底仓；单日涨幅 > 5% 时，追高买入 $50,000；单日跌幅 > 5% 时，杀跌卖出 $50,000；追涨杀跌，直到现金耗尽或空仓。",
    gambler: "每支买入 $10,000 底仓；现价 < 上次成交价 * 0.90 时，双倍金额补仓；现价 > 平均成本 * 1.01 时，全部卖出；亏损无上限，直到 $1,000,000 本金耗尽。",
    mom: "每支买入 $200,000 满仓，不留现金；现价 > 上次成交价 * 1.20 时，卖出 20% 仓位；现价 < 上次成交价 * 0.95 时，全部清仓离场；不设加仓逻辑。",
    dog: "每支买入 $40,000 底仓，保留 $800,000 现金；现价 > 买入价 * 1.05 时，卖出 50% 仓位；现价 < 买入价 * 0.98 时，全部清仓止损；永不追加买入。",
    xiaoqing: "每支买入 $100,000 底仓；现价 < 上次成交价 * 0.85 时，买入 $50,000；无论涨跌，永不卖出；直到 $1,000,000 本金全部用完。",
    soldier: "每支买入 $100,000 底仓；现价 < 上次成交价 * 0.98 时，买入 $10,000；现价 > 上次成交价 * 1.02 时，卖出 20% 仓位；总回撤 > 10% 时，停止所有买入操作。",
    zen: "每支买入 $100,000 底仓；每日对每股随机买入或卖出 $10,000；无视价格波动，直到本金或持仓耗尽。" // 👈 新增这一行
  };
interface Props {
  portfolios: any[];
